...
`API_URL` hace falta en el **build** (`/llms.txt` se prerrenderiza con los precios dentro) y otra vez en **ejecución** (de ahí sale cada revalidación). Y la resuelve el proceso de Next, no el navegador: desde el host `localhost` es la api del host, pero dentro del contenedor hay que apuntar a `http://api:21001/api/v1`.
