...
│   ├── icon.png    #   icono del navegador (512) — convención de archivo
│   ├── globals.css
│   ├── robots.ts   #   /robots.txt      — generados por Next, no hay archivos
│   ├── sitemap.ts  #   /sitemap.xml       estáticos en public/ para esto
│   ├── manifest.ts #   /manifest.webmanifest
│   ├── llms.txt/   #   /llms.txt        — el ÍNDICE de enlaces, para modelos
│   ├── llms-full.txt/  # /llms-full.txt — el sitio ENTERO en texto plano
...
├── lib/            # utils.ts (cn), currency.ts, region.ts, textos.ts, legal.ts, llms.ts, seo.ts (JSON-LD + hreflang), movimiento.ts (entradas por scroll)
│   └── api/        #   └─ cliente.ts (el único fetch a la api) + endpoints.ts
├── types/          # landing.ts — lo que devuelve la API pública
└── public/         # los webp de marca (logotipo y marca, en claro y oscuro), la sala del hero,
                    # og.jpg (la tarjeta social) e icono-192/512.png (los del manifiesto)
```
...
- **Cada sección del navbar es una PÁGINA, no un ancla.** Producto, cómo se ve, precios y preguntas viven en `/[idioma]/…`; el navbar enlaza con `next/link` y marca la activa con `usePathname()`. Antes eran anclas de la portada y el activo salía de un `IntersectionObserver` sobre sus elementos: **desde cualquier página que no fuera la portada —las legales, por ejemplo— el observador no encontraba nada y los enlaces no llevaban a ninguna parte.** La cabecera se veía entera y no funcionaba.
  ...
  - Desde `lg` la cabecera es una **rejilla `1fr auto 1fr`**, no una fila flexible. Las dos columnas laterales se reparten el sobrante a partes iguales, así que la pastilla cae en el centro de la PANTALLA. Con `mx-auto` en una fila se centraba respecto al hueco libre —y con el logotipo a un lado y tres botones al otro, eso no es el centro—; posicionada en absoluto se centraba bien pero no reservaba espacio y a 1024 px se solapaba con los botones.
  - **Por debajo de `lg` es una fila flexible**, porque ahí no hay pastilla: la columna del centro quedaba vacía pero sus dos huecos seguían ocupando 32 px. En el teléfono la barra mide `h-16`, la llamada es `h-9` con su etiqueta corta (`navegacion.empezarCorto`) y sin flecha, y el logotipo baja un paso por debajo de 360 px. Con las medidas de escritorio, logotipo + llamada + menú pedían ~420 px en una fila de ~320: todo se apretaba y los botones se veían desproporcionados. **Si cambia el alto de la barra, cambia con él el `pt-*` de las páginas sin hero** y el `scroll-mt-*` de los documentos legales.
  ...
  - **El menú móvil ocupa la pantalla entera bajo la barra**, con lo secundario (legal, idioma, tema, entrar) anclado abajo, y la barra es sólida mientras está abierto. A media altura, lo de debajo era página difuminada y se leía como un bloque vacío. **El velo es `bg-velo`, un token oscuro en los dos temas**: con `foreground/50`, en oscuro salía casi blanco y blanqueaba la mitad de la pantalla.
...
- **El texto sale del diccionario, y el diccionario se resuelve en el SERVIDOR.** `obtenerTextos(idioma)` a partir del segmento; los componentes cliente reciben su porción por props. Sin proveedor, sin store y sin hidratación — que es la diferencia grande con el panel, y viene de que aquí el idioma está en la URL.
  ...
  - **En el texto visible es «página de reservas», nunca «escaparate».** En Colombia «escaparate» se lee como la vitrina de una tienda y no dice que ahí se reserva. En el código y en las claves del diccionario sigue siendo `escaparate`, que es el nombre del dominio en todo el sistema; lo que cambia es lo que lee el dueño de la barbería. Vale también para los tres documentos legales (versión `2026-09-10`).
  - **«Celular», no «móvil».** Es como se dice en Colombia, que es donde opera Barion. «Móvil» se entiende, pero se lee a España, y el sitio no debería sonar traducido para su propio mercado.
...
- **Título, descripción, canónica, `hreflang` y tarjeta social salen de UNA función**, `metadatosDePagina(idioma, clave)` de `lib/seo.ts`, y cada página la llama con su clave. No es azúcar: Next **sustituye `openGraph` entero, no lo fusiona**, exactamente igual que hace con `alternates`, así que el declarado en el layout se heredaba tal cual y toda subpágina se compartía con el título de la portada y con un `og:url` que contradecía a su propia canónica. Escribiéndolas juntas y desde la misma `rutaDe()`, `og:url` y canónica no pueden discrepar. Y siguen sin declararse en el layout —donde el 404 las heredaría y pediría indexar la portada una vez por cada enlace roto—: lo que queda arriba es solo el respaldo del 404.
- **`/robots.txt`, `/sitemap.xml`, `/manifest.webmanifest`, `/llms.txt` y `/llms-full.txt` los genera Next** (`app/robots.ts`, `app/sitemap.ts`, `app/manifest.ts` y las dos routes sobre `lib/llms.ts`), nunca archivos estáticos en `public/`: los precios y el texto salen de las constantes y de la API, así que un archivo escrito a mano miente en la primera subida de precios.
- **`/llms.txt` es un ÍNDICE DE ENLACES y `/llms-full.txt` el texto completo**, y son dos porque la convención de `llmstxt.org` espera lo primero: un `# H1`, un resumen en cita y secciones `##` cuyo contenido son listas `- [Nombre](url): descripción`. Con las direcciones escritas en crudo, un cliente que sigue la convención al pie de la letra no encontraba qué páginas existen — que es justo para lo que está el archivo. Los dos los escribe `lib/llms.ts`: dos copias del texto se desincronizan al primer plan nuevo. Los dos llevan **fecha de última actualización**, porque un asistente que cita necesita poder decir de cuándo es lo que cita.
- **El `lastmod` del sitemap es una fecha ESCRITA, no la del build.** Con `new Date()` las dieciséis direcciones cambiaban a la vez cada vez que se subía cualquier cosa —una dependencia incluida—, y un mapa que dice que todo cambió ayer deja de ser una señal. Cada página lleva la suya en `PAGINAS` y se mueve al editar su texto; la de los tres documentos legales sale de `VERSION_LEGAL`, que ya es esa fecha.
...
- **Cada página declara su `BreadcrumbList` y su `WebPage`** con `grafoPagina()`. El primero es lo que hace que un resultado profundo salga como «Barion › Legal › Términos» en vez de como una URL cruda; el segundo cierra el grafo, que hasta ahora dejaba nodos sueltos sin decir de qué sitio formaban parte. **Un escalón sin página —«Legal», que agrupa tres documentos y no es ninguno— se pinta en `Migas` pero NO entra en el JSON-LD**: un `ListItem` intermedio sin `item` es una posición de la ruta a la que no se puede ir. Lo visible explica; lo estructurado se navega.
- **La tarjeta social es `public/og.jpg`, 1200×630 y JPEG.** No se reutiliza el webp del hero: WhatsApp, LinkedIn y varios clientes de correo no pintan WebP en la vista previa y la tarjeta se cae al texto sin imagen, y el 3:2 se recortaba por arriba y por abajo sin que el recorte lo eligiera nadie.
- **La verificación de propiedad (`GOOGLE_SITE_VERIFICATION`, `BING_SITE_VERIFICATION`) va en `env.server.ts`, sin `NEXT_PUBLIC_`.** Acaba escrita en el HTML, pero quien la pinta es `generateMetadata`, que corre en el servidor; con prefijo viajaría además en el bundle. Sin código configurado no se pinta la etiqueta: una vacía no verifica nada y hace fallar la comprobación con un mensaje que no dice por qué.
  - **Hacen falta en los DOS lados, y ese es el detalle que se olvida.** Como **build arg** (`Dockerfile` + `docker-compose.prod.yml`) porque `/producto`, `/vista-previa`, `/preguntas` y las tres legales se prerrenderizan: ahí la etiqueta se escribe al construir. Y en el **`env_file`** porque la portada y `/precios` leen cookies y cabeceras, así que son dinámicas y resuelven su metadata en cada petición. Con una sola de las dos, la mitad de las páginas sale sin etiqueta — y el contenedor arranca sano, así que solo se ve mirando el HTML servido.
  - **El método de archivo de Bing (`BingSiteAuth.xml`) NO se usa, y no es un olvido.** Lleva el mismo código que la etiqueta, así que serían dos sitios donde escribir lo mismo y uno de los dos se quedaría viejo. La etiqueta sale de la variable, que ya viaja por el build y por el `env_file`; el archivo habría que acordarse de moverlo a mano. Si Bing vuelve a ofrecerlo, la respuesta es la etiqueta.
- **`browserslist` está fijado en `package.json` al mismo suelo que pide Tailwind 4** (Chrome/Edge 111, Firefox 128, Safari 16.4). Sin él, el build transpilaba `Array.prototype.at/flat/flatMap` para navegadores que este sitio no tiene que soportar: 13 KiB de JavaScript antiguo en la primera pantalla. El CSS del sitio ya exige ese suelo (`interpolate-size`, `::details-content`), así que bajarlo no ampliaría el soporte, solo el peso.
...
- **Iconos del navegador: `app/favicon.ico`, `app/apple-icon.png` y `app/icon.png`, por convención de archivo.** Ni campo `icons` en la metadata (duplicaría los `<link>`) ni favicon en webp: Safari no lo pinta y Google no lo acepta para el icono del resultado. Los del manifiesto (`public/icono-192.png`, `public/icono-512.png`) son otros: los referencia `app/manifest.ts`, no una etiqueta. Los webp de `public/` son el logotipo DENTRO de la página, que es otra cosa. Un asset en `public/` que no referencie ningún componente ni una de esas rutas sobra — se borra.
- **Animaciones con `motion`**: todo el árbol va bajo el `MotionConfig reducedMotion="user"` del layout — no anidar otro. Micro-interacciones 150-300ms.
  - **Las entradas por scroll salen de `lib/movimiento.ts`, nunca escritas a mano**: curva de salida larga (1 s, `CURVA_SUAVE`), recorrido corto (20 px), cascada de 0,12 s y disparo con el bloque ya un 15 % por encima del borde (`VISTA_REVELAR`). Eran resortes 140/22 con 28-32 px disparados en el borde, y al bajar cada sección se encendía de golpe antes de que llegara la vista.
  - **El retardo y la cascada van DENTRO de la variante.** Motion usa la transición de la variante y descarta la del componente: el `delay` que se pasaba por `transition` no llegaba nunca, y los hermanos que tenían que escalonarse entraban a la vez.
  - **Nada entra de lado.** Un bloque que arranca desplazado en horizontal asoma por el borde en el teléfono y le da a la página scroll horizontal; `RevelarEnScroll` solo sube, se amplía o funde. Por lo mismo `<main>` lleva `overflow-x-clip`, y la barra de la página se pinta con los tokens `--barra`/`--barra-carril`, no con la del sistema.
- **Lo que se ve SIN desplazar la página no arranca en `opacity: 0` por obra de un componente cliente.** El hero entraba con `variants` de Motion, así que el HTML del servidor lo traía transparente y no se veía hasta que el bundle bajaba, se ejecutaba e hidrataba: el LCP medido era el párrafo de entrada, con **3.170 ms de retraso de renderizado** sobre un texto que ya venía escrito. Un navegador no cuenta como pintado lo que es invisible. Ahí la entrada se hace con CSS (`.entra-hero` + `@keyframes`, con el retardo en `--retardo`), que arranca en el primer fotograma. Motion se queda para lo que solo actúa al desplazar —el parallax, la pista de scroll— y `RevelarEnScroll` para las secciones que no se ven de entrada, que es donde está bien.
- **Una opacidad a medias es un color distinto, y el contraste se comprueba contra el que se VE.** Las citas de la maqueta del hero fundían cada una por su cuenta sobre un panel que ya fundía entero, y el auditor las capturaba a medio camino: 3,04:1 donde el token da cerca de 9:1. El escalonado se hace con `transform`, que no toca el color, y las atenuaciones permanentes se quedan en 0,75 — por debajo, el gris efectivo baja del mínimo legible.
- **El nivel de un encabezado se DERIVA del de su sección**, nunca se escribe fijo. Las preguntas estaban clavadas en `h4` y el orden dependía de la página: en la portada la sección abre con `h2`, no hay grupos, y salían `h4` sin ningún `h3` en medio. Un lector de pantalla salta por títulos, así que una jerarquía con huecos es la navegación rota.
- **Un envoltorio de animación dentro de una lista ES el `<li>`, no se mete en medio.** `RevelarEnScroll` acepta `como="li"` justo para eso: un `<div>` entre la `<ol>` y sus elementos deja la lista con hijos que no son elementos de lista, y para un lector de pantalla los tres pasos dejan de ser una lista.
