...
- **Lo legal se publica AQUÍ, y es el único contenido del sitio que otro repo consume.** Términos (con el anexo de encargo dentro), privacidad y cookies viven en `app/legal/`, se escriben como datos en `constants/legal/` —igual que el FAQ, y por lo mismo: más de un destino— y los pinta un solo `components/legal/DocumentoLegal.tsx`. Se indexan a propósito: quien los busca todavía no tiene cuenta con la que entrar a leerlos.
...
- **El texto sale del diccionario, y el diccionario se resuelve en el SERVIDOR.** `obtenerTextos(idioma)` a partir del segmento; los componentes cliente reciben su porción por props. Sin proveedor, sin store y sin hidratación — que es la diferencia grande con el panel, y viene de que aquí el idioma está en la URL.
  - **`config/contenido.ts` tiene la ESTRUCTURA** (qué bloques hay, en qué orden, con qué icono) y **`constants/textos/` el TEXTO**. Un icono no se traduce y un orden tampoco; si vivieran en el diccionario habría que repetirlos por idioma, y el día que se reordenen en español el inglés se queda con el orden viejo sin que nada avise.
  - Los diccionarios se declaran contra la interfaz de `constants/textos/tipos.ts`, no derivando el tipo del español: así **falta una clave y no compila** — en el idioma que sea, y también en el base.
