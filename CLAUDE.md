...
- **Un servicio decide el respaldo, y lo AVISA.** Mientras `GET /publico/planes` no exista, `services/planes.ts` sirve `constants/planes.fallback.json` con un `console.warn`. Sin ese aviso, una api caída se ve igual que una api sana y nadie se entera de que la página lleva un mes con precios congelados.
